...
11. 如何把函数绑定到事件上：
//方法一  
$('#foo').click(function(event) { 　　alert('User clicked on "foo."'); });   
//方法二, 支持动态传参  
$('#foo').bind('click', {test1:"abc", test2:"123"}, function(event) { 　　alert('User clicked on "foo."' + event.data.test1 + event.data.test2 ); });  
...
48. 如何检查cookie是否启用
var dt = new Date(); dt.setSeconds(dt.getSeconds() + 60); document.cookie = "cookietest=1; expires=" + dt.toGMTString(); var cookiesEnabled = document.cookie.indexOf("cookietest=") != -1; if(!cookiesEnabled) { 　　  
//没有启用cookie   
}   
...
$.fn.replaceUrl = function() { 　　var regexp = /((ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(: 
```