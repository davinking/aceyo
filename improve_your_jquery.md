...
目录
  ...
  1. 使用备忘单
  1. 整合所有的脚本并缩减它们
  1. 使用Firebug出色的控制台日志工具
  1. 通过缓存最小化选择操作
  1. 最小化DOM操作
  1. 处理DOM插入操作时，将需要的内容包装在一个元素中
  1. 尽可能使用IDs而不是classes
  1. 给选择器提供上下文
  1. 正确使用方法链
  1. 学会正确使用效果
  1. 了解事件代理
  1. 利用classes存储状态
  1. 更好的方法是利用jQuery内置的data()方法存储状态
  1. 写你自己的选择器
  1. 精简你的HTML并在页面加载后修改它
  1. 为了速度和SEO方面的考虑，延迟加载内容
  1. 使用jQuery提供的工具函数
  1. 使用noConflict重命名jQuery对象
  1. 如何得知图片已加载完毕
  1. 总是使用最新版本
  1. 如何检查元素是否存在
  1. 给你的HTML属性增加JS类
